 ...
 * 
 * Time complexity: O(n)
 * The time complexity would be O(n + m), where 'n' is the length of the string and 'm' is the count of unique characters. 
 * However, given that the number of unique characters (m) is capped by the size of the alphabet, which is a constant, 
 * it doesn't grow with the input size. Therefore, we can simplify the time complexity to O(n).
*/
